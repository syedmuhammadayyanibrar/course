...
the keys k in K that are close to q. For this queries q and keys k are hashed into bucket using LSH.Attention is computed within each bucket (not individually selecting neighbors per query).
