...
We pass the option `grouped_entities=True` in the pipeline creation function to tell the pipeline to regroup together the parts of the sentence that correspond to the same entity: here the model correctly grouped "Hugging" and "Face" as a single organization, even though the name consists of multiple words. In fact, as we will see in the next chapter, the preprocessing even splits some words into smaller parts. For instance, `Sylvain` is split into four pieces: `S`, `##yl`, `##va`, and `##in`. In the post-processing step, the pipeline successfully regrouped those pieces. In later versions of tensorflow 'grouped_entities=True' is replaced by 'aggregation_strategy'. it has following different possible values.
  "none" – Returns raw token-level predictions without any merging; each subword token is labeled independently.
  "simple" – Groups consecutive tokens with the same entity label into a single span, averaging their scores; does not strictly enforce BIO rules.
  "first" – Assigns the entity label of the first subword token to the whole word, ignoring the rest.
  "average" – Averages the scores of all subword tokens in a word, then selects the highest-scoring label.
  "max" – Selects the label of the subword token with the highest score within the word.
